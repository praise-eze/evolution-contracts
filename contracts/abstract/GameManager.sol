...
abstract contract GameManager {
  ...
  function getGameManager() external view virtual responsible returns (address gameManager) {
  ...
  function getMetadataAccess() external view virtual responsible returns (bool metadataAccess) {
    return { value: 0, flag: 64, bounce: false } (_metadataAccess);
  }

  /// @notice Modifier for access to change state of attribute params
  /// @param _metadataAccess_ Expected value of `_metadataAccess`
  modifier metadataAccess(bool _metadataAccess_) virtual {
    require(_metadataAccess == _metadataAccess_, 101);
    _;
  }
